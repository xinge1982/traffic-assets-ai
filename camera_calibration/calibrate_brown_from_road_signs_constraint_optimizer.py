...
def main():
    ...
    base=np.asarray([args.initial_focal,args.initial_focal,w/2,h/2,0,0,0,0,0,args.yaw_offset,args.pitch,args.roll,args.lever_forward,args.lever_right,args.lever_up],float);prior=base.copy()
    ...
    free_ext=[] if args.fix_extrinsics else list(range(9,15));candidates=[]
    ...
    for yaw in yaw_starts:
        ...
        try:
            p,sol=staged_fit(p0,train,lo,hi,args,prior,free_ext);m=stats(p,val,args.min_depth);score=m["radial_pixels"]["median_abs"] if m["front_count"]>=max(4,.8*len(val["u"])) else float("inf");candidates.append((score,p,sol))
    ...
    span=hi-lo;hit=[NAMES[i] for i in range(15) if (best[i]-lo[i])/span[i]<.005 or (hi[i]-best[i])/span[i]<.005]
    warnings=[]
    ...
    out={"opencv":{"camera_matrix_K":K,"dist_coeffs_D_k1_k2_p1_p2_k3":D},"parameters":dict(zip(NAMES,map(float,best))),"parameter_bounds":{"lever_forward_m":[args.lever_forward_min,args.lever_forward_max],"lever_right_m":[args.lever_right_min,args.lever_right_max],"lever_up_m":[args.lever_up_min,args.lever_up_max]},"coordinates":{k:v for k,v in coord.items() if k!="rejected_input"},"split":{"unique_signs":len(ids),"train_rows":len(train["u"]),"validation_rows":len(val["u"]),"validation_signs":nv,"seed":args.seed},"train":stats(best,train,args.min_depth),"validation":vmtr,"plot":plot,"optimizer":{"success":bool(sol.success) if sol else True,"message":sol.message if sol else "fixed","successful_starts":len(candidates)},"warnings":warnings,"rejected_input_preview":coord["rejected_input"][:10]}
