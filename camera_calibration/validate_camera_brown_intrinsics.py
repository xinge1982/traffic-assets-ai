#!/usr/bin/env python3
"""Validate supplied pinhole + Brown-Conrady parameters using all observations.
...
attitude, project with supplied K and k1/k2/p1/p2/k3, calculate pixel errors,
...
def brown_project_numpy(point_xyz: np.ndarray, camera_matrix: np.ndarray,
                        dist_coeffs: np.ndarray) -> tuple[float, float]:
    """Reference Brown-Conrady implementation matching cv2.projectPoints."""
    x, y, z = map(float, point_xyz)
    fx, fy = camera_matrix[0, 0], camera_matrix[1, 1]
    cx, cy = camera_matrix[0, 2], camera_matrix[1, 2]
    k1, k2, p1, p2, k3 = map(float, dist_coeffs)
    xn, yn = x/z, y/z
    r2 = xn*xn + yn*yn
...
def make_brown_projector(args):
    """Build the selected Brown-Conrady projector and its explicit K/D arrays."""
    camera_matrix = np.asarray([[args.fx, 0.0, args.cx],
                                [0.0, args.fy, args.cy],
                                [0.0, 0.0, 1.0]], dtype=np.float64)
    # OpenCV Brown-Conrady order is exactly: k1, k2, p1, p2, k3.
    dist_coeffs = np.asarray([args.k1, args.k2, args.p1, args.p2, args.k3],
                             dtype=np.float64)
    if args.projection_engine == "opencv":
        try:
            import cv2
        except ImportError as exc:
            raise RuntimeError(
                "OpenCV is required for --projection-engine opencv; "
                "install it with: pip install opencv-python") from exc
        zero = np.zeros((3, 1), dtype=np.float64)
        def project(point_xyz):
            # XYZ is already in the camera frame, therefore rvec=tvec=0.
            object_point = np.asarray(point_xyz, dtype=np.float64).reshape(1, 1, 3)
            image_points, _ = cv2.projectPoints(
                object_point, zero, zero, camera_matrix, dist_coeffs)
            u, v = image_points.reshape(-1, 2)[0]
            return float(u), float(v)
        return project, camera_matrix, dist_coeffs, "cv2.projectPoints"
    def project(point_xyz):
        return brown_project_numpy(point_xyz, camera_matrix, dist_coeffs)
    return project, camera_matrix, dist_coeffs, "NumPy Brown-Conrady reference"
...
def main() -> int:
    ...
    ap.add_argument("--k1",type=float,default=0.0); ap.add_argument("--k2",type=float,default=0.0)
    ap.add_argument("--p1",type=float,default=0.0); ap.add_argument("--p2",type=float,default=0.0)
    ap.add_argument("--k3",type=float,default=0.0)
    ap.add_argument("--projection-engine",choices=("opencv","numpy"),default="opencv",
                    help="opencv uses cv2.projectPoints; numpy is a dependency-light reference")
    ...
    args=ap.parse_args()
    projector, camera_matrix, dist_coeffs, projection_engine = make_brown_projector(args)
    raw=[]; rejected=[]
    ...
    for line,fid,camera,sign,heading,u,v in raw:
        try:
            ...
            # This is the actual K + Brown-Conrady projection step.
            up,vp=projector(np.asarray([x,y,z],dtype=np.float64))
            du=up-u; dv=vp-v
    ...
    report={"camera_model":"pinhole + Brown-Conrady",
            "projection_engine":projection_engine,
            "distortion_order":["k1","k2","p1","p2","k3"],
            "camera_matrix":camera_matrix.tolist(),
            "intrinsics":{"fx":args.fx,"fy":args.fy,"cx":args.cx,"cy":args.cy},
            "distortion":{"k1":args.k1,"k2":args.k2,"p1":args.p1,"p2":args.p2,"k3":args.k3},
