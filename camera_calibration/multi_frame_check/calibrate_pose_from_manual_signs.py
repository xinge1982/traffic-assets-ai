#!/usr/bin/env python3
"""Calibrate camera pose/GNSS alignment from manually matched road signs.
...
and writes per-point camera XYZ and reprojection residuals.

The recommended ``pose_only`` mode fixes K and Brown distortion, then estimates
only a shared yaw correction and a shared along-track photo/GNSS offset.  This
avoids letting focal length, principal point, distortion, and pose compensate
for one another.  The original full calibration remains available as ``full``.
...
Along-track offset: positive moves the effective camera position forward along
the recorded vehicle heading. Coordinates are metres after WGS84 -> local UTM.
"""
...
def residual(p, d, min_depth, prior, width, height, components):
    up, vp, z, _ = project(p, d, min_depth)
    ru = np.clip(up - d["u"], -5000, 5000);
    rv = np.clip(vp - d["v"], -5000, 5000)
    bad = np.maximum(0, min_depth - z) * 1000
    if components == "u":
        # First-stage yaw/position calibration uses horizontal reprojection.
        # Vertical pixels also depend on pitch, road grade and height datum,
        # which are deliberately fixed in pose_only mode.
        data = ru + bad
    else:
...
def fit_stage(p, active, d, lo, hi, args, prior, tie_f=False):
    ...
    sol = least_squares(lambda x: residual(unpack(x), d, args.min_depth, prior,
...
def staged_fit(p, train, lo, hi, args, prior, free_ext, free_track):
    ...
    if args.fit_mode == "pose_only":
        # Parameter 9: yaw_offset_deg; parameter 15: along_track_offset_m.
        # Everything else stays at the supplied fixed value.
        return fit_stage(p, [9, 15], train, lo, hi, args, prior, tie_f=False)
...
def main():
    ...
    ap.add_argument("--along-track-offset", type=float, default=0.0,
                    help="initial global frame/GNSS position offset along camera_heading in metres")
    ap.add_argument("--along-track-offset-min", type=float, default=-30.0,
                    help="lower bound for global along-track offset in metres (default: -30)")
    ap.add_argument("--along-track-offset-max", type=float, default=30.0,
                    help="upper bound for global along-track offset in metres (default: 30)")
    ap.add_argument("--fix-along-track-offset", action="store_true",
                    help="keep the supplied global along-track offset fixed")
    ap.add_argument("--fix-extrinsics", action="store_true", help="keep supplied yaw/pitch/roll/lever fixed")
    ap.add_argument("--fit-mode", choices=("full", "pose_only"), default="pose_only",
                    help="pose_only fixes K/D/pitch/roll/lever and fits only yaw + along-track (default)")
    ap.add_argument("--fixed-fx", type=float, default=1921.8,
                    help="fixed horizontal focal length in pixels for pose_only")
    ap.add_argument("--fixed-fy", type=float, default=1921.8,
                    help="fixed vertical focal length in pixels for pose_only")
    ap.add_argument("--fixed-cx", type=float, default=1752.0,
                    help="fixed principal point x in pixels for pose_only")
    ap.add_argument("--fixed-cy", type=float, default=1465.8,
                    help="fixed principal point y in pixels for pose_only")
    ap.add_argument("--residual-components", choices=("auto", "u", "uv"), default="auto",
                    help="auto uses u for pose_only and u+v for full calibration")
    ...
    ap.add_argument("--exclude-feature-id", action="append", default=[],
                    help="feature_id to exclude from fitting/evaluation; repeat for multiple IDs")
    ...
    args = ap.parse_args()
    if args.residual_components == "auto":
        args.residual_components = "u" if args.fit_mode == "pose_only" else "uv"
    if args.fit_mode == "pose_only" and (args.fix_extrinsics or args.fix_along_track_offset):
        ap.error("pose_only must optimize yaw and along-track; do not pass --fix-extrinsics or "
                 "--fix-along-track-offset")
    for name in ("fixed_fx", "fixed_fy"):
    ...
    if args.exclude_feature_id:
        excluded_ids = set(map(str, args.exclude_feature_id))
        keep = np.asarray([str(x) not in excluded_ids for x in d["id"]])
        excluded_rows = int((~keep).sum())
        d = subset(d, keep)
        coord["explicit_exclusions"] = {
            "feature_ids": sorted(excluded_ids),
            "row_count": excluded_rows,
        }
        if len(d["id"]) < 20:
            ap.error("fewer than 20 rows remain after --exclude-feature-id filtering")
    ...
    w, h = args.image_width, args.image_height
    if args.fit_mode == "pose_only":
        base = np.asarray([args.fixed_fx, args.fixed_fy, args.fixed_cx, args.fixed_cy,
                           0, 0, 0, 0, 0,
                           args.yaw_offset, 0, 0, 0, 0, 0,
                           args.along_track_offset], float)
    else:
        base = np.asarray([args.initial_focal, args.initial_focal, w / 2, h / 2,
                           0, 0, 0, 0, 0,
                           args.yaw_offset, args.pitch, args.roll,
                           args.lever_forward, args.lever_right, args.lever_up,
                           args.along_track_offset], float)
    prior = base.copy()
    ...
    if free_track and 12 in free_ext:
        free_ext.remove(12)
    candidates = []
    yaw_starts = ([args.yaw_offset] if args.fix_extrinsics else
                  np.linspace(-.8 * args.yaw_limit, .8 * args.yaw_limit, args.multistart))
    for yaw in yaw_starts:
        p0 = base.copy();
        p0[9] = yaw
        try:
            p, sol = staged_fit(p0, train, lo, hi, args, prior, free_ext, free_track);
            train_metrics = stats(p, train, args.min_depth)
            metric = "u_pixels" if args.residual_components == "u" else "radial_pixels"
            # Multistart selection must use training data only.  Validation
            # signs remain genuinely unseen until the final report.
            score = (train_metrics[metric]["median_abs"]
                     if train_metrics["front_count"] >= max(4, .8 * len(train["u"]))
                     else float("inf"))
    ...
    span = hi - lo;
    hit = [NAMES[i] for i in range(len(NAMES)) if
           (best[i] - lo[i]) / span[i] < .005 or (hi[i] - best[i]) / span[i] < .005]
    warnings = []
    ...
    if free_track: warnings.append(
        "lever_forward_m was held fixed because it is exactly degenerate with along_track_offset_m")
    ...
    out = {"fit_mode": args.fit_mode,
           "residual_components": args.residual_components,
           "optimized_parameters": (["yaw_offset_deg", "along_track_offset_m"]
                                    if args.fit_mode == "pose_only" else "full staged parameter set"),
           "selection_score": {"metric": ("training_u_median_abs_px"
                                             if args.residual_components == "u"
                                             else "training_radial_median_abs_px"),
                               "value": float(score)},
           "baseline_before_optimization": {"train": stats(base, train, args.min_depth),
                                              "validation": stats(base, val, args.min_depth)},
           "opencv": {"camera_matrix_K": K, "dist_coeffs_D_k1_k2_p1_p2_k3": D},
           "parameters": dict(zip(NAMES, map(float, best))),
           "position_offset": {"installation_lever_forward_m": float(best[12]),
                               "global_along_track_offset_m": float(best[15]),
                               "effective_forward_offset_m": float(best[12] + best[15])},
           "parameter_bounds": {"lever_forward_m": [args.lever_forward_min, args.lever_forward_max],
                                ...
                                "along_track_offset_m": [args.along_track_offset_min, args.along_track_offset_max]},
